...
function getLoginForm (conf, body) {
  var LOGIN_FORM_SELECTOR = conf['loginFormSelector']
  if (!LOGIN_FORM_SELECTOR) {
    var LOGIN_FORM_ID = conf['loginFormId']
...
function getCsrfTokenName (conf) {
  var CSRF_TOKEN_NAME = conf['tokenFieldName']
...
function csrfLogin (options) {
  ...
  function login (csrfInfo, answers) {
    ...
    var usernameField = conf['loginUsernameField'] || 'email'
    var passwordField = conf['loginPasswordField'] || 'password'
